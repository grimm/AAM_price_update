...
def do_ampm(vendor_pandas, tech_cal):
    ...
    vendor_pandas["P1"] = vendor_pandas["2018 MSRP"].replace("$", "")
    vendor_pandas["P1"] = vendor_pandas["P1"].replace("", "0").astype(float)

    vendor_pandas["P3"] = vendor_pandas["2018 Jobber"].replace("$", "")
    vendor_pandas["P3"] = vendor_pandas["P3"].replace("", "0").astype(float)

    vendor_pandas["P2"] = vendor_pandas["2018 Unilateral Suggested Minimum Retail Price"].replace("$", "")
    vendor_pandas["P2"] = vendor_pandas["P2"].replace("", "0").astype(float)

    vendor_pandas["P4"] = vendor_pandas["2018 Unilateral Suggested Minimum Wholesale Price"].replace("$", "")
    vendor_pandas["P4"] = vendor_pandas["P4"].replace("", "0").astype(float)
