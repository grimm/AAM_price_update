...
def do_kar(vendor_pandas, tech_cal):
    ...
    frames = [vendor_pandas["Parts Prices"], vendor_pandas["Package Prices"]]
    vendor_pandas = pd.concat(frames, axis=0)
    ...
    vendor_pandas["P2"] = vendor_pandas["Trade Price"].replace("$","").replace(",","")
    vendor_pandas["P2"] = vendor_pandas["P2"].replace(",","").astype(float)
    vendor_pandas["P5"] = vendor_pandas["Your Cost"].replace("$","")
